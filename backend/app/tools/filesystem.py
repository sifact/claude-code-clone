...
from .sandbox import MAX_FILE_SIZE, MAX_MATCHES, MAX_RESULTS, WORKDIR, ToolError, resolve_inside_workdir


def read_file(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input["path"])
...
def list_directory(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input.get("path", "."))
    entries = []
...
def glob_files(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input.get("path", "."))
...
def grep_files(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input.get("path", "."))
...
def write_file(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input["path"])
...
def edit_file(tool_input: dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_inside_workdir(tool_input["path"])
    content = resolved.read_text(encoding="utf-8")
